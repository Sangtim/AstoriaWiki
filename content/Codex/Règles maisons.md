...
Lorsqu'un jet de discrétion sera nécessaire ou demandé, les règles suivantes seront appliquées :
- **Réussite totale** : Si tous les joueurs concernés par le jet le réussisse, on procède comme dans les règles établis.
- **Echec total** : Si tous les joueurs concernés ratent leur jet, on procède comme dans les règles établis.
- **Echec partiel** : Si un ou plusieurs joueurs ratent leurs jets mais qu'au moins un joueur du groupe réussit alors :
	- **Caché** : Les joueurs ayant réussi leur jet doivent l'avoir réussi avec +2 par rapport à la difficulté sous peine d'être dévoilé.
	- **Aide** : Les joueurs encore caché après la règle suivante peuvent alors utiliser leur réaction pour aider les joueurs dévoilé. Ils peuvent alors effectuer un autre jet de discrétion plus difficile (DC 8 + différence entre jet du membre dévoilé et l'ancien DC). Si ils réussissent, le membre qu'ils aident restent caché. En cas d'échec, le membre qui aide est lui aussi dévoilé.

**Exemple** : 
- Célestin et Tifanny font un jet de discrétion (DC 14). Célestin fait un 8 et Tifanny fait un 17.
	Célestin est dévoilé et Tifanny reste caché. 
	Tifanny décide d'aider Célestin à rester caché, elle lance un nouveau jet de discrétion (DC 8 + 6 = DC  14) et elle fait 18. 
...
- **Attaque imparable** : Si le jet d'attaque est réussi avec plus de 10 points par rapport à la cible, ce jet est automatiquement considéré comme critique, même sans avoir fait 20.
	**Exemple** : Un joueur attaque une créature avec une classe d'armure à 12. Le joueur fait 18 et à +4 sur son jet, faisant un total de 22. Ce total étant à +10 par rapport à la cible (AC 12), l'attaque est considéré comme un critique.
...
- **Positionnement** :
	- **Encerclement** : Attaquer une créature déjà au contact d'un de vos alliés accordera un désavantage de -2 à sa classe d'armure. 
		- Attention, valable pour les ennemis aussi.
	- **The High Ground** : Attaquer depuis un endroit surélevé accorde un malus de -2 à la classe d'armure de la cible. Inversement, attaquer vers un endroit surélevé accorde un avantage de +2 à sa classe d'armure.
	- **Opportunité** : Les attaques d'opportunité disparaissent totalement, pour les aventuriers comme pour les ennemis. Les Dons et autres caractéristiques de classes utilisant les attaques d'opportunités permettent cependant de les réactiver et de profiter des bonus correspondants. 
		- **Interposer** : A la place, si un ennemi passe à côté de vous, vous pouvez vous interposer entre l'ennemi et sa cible et essayer de l'attraper demandant un jet de force ou de dextérité selon le cas. Cela vous expose à l'attaque de l'ennemi.
		- **Disclaimer** : Un ennemi qui passe dans la zone de corps à corps de deux combattants de mêlée se verra tout de même infliger une attaque de la part d'un des deux combattants au choix. 

- **Initiative** : Lors des jets d'initiative, un joueur peut demander d'utiliser une autre statistiques que la Dextérité si il arrive à le justifier par rapport à la situation pré-combat.
	**Exemple** : Un clerc ayant déceler l'embuscade au préalable peut demander à utiliser sa statistiques de Sagesse pour son jet d'initiative ou un magicien ayant défini un plan d'attaque peut demander à utiliser sa statistiques d'Intelligence.
...
- **Athéisme** : Si un des joueurs de l'équipe est athéiste, il pourra quand même accumulé des points de piété en réalisant des actions courageuses et repoussant les limites de ses capacités, pouvant ainsi acquérir des compétences liés à son personnage.